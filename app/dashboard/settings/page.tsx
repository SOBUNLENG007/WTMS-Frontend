...
export default function SettingsPage() {
  ...
  // Fetch and seed form with current user profile from backend
  useEffect(() => {
    if (!hydrated || !user) return;
    (async () => {
      try {
        const response = (await settingsService.getProfile()) as any;
        // If the backend response is wrapped (e.g., { payload: {...} }), extract it
        ...
        setProfile({
          ...
          birthDate: p.birthDate || p.dob || "", // Not present in backend, will be blank
          gender: p.gender || "", // Not present in backend, will be blank
          address: p.address || p.departmentName || "",
        });
        // If backend returns avatarUrl, use it; else use default
        if (p.avatarUrl) {
  ...
  }, [hydrated, user, nameParts, defaultAvatar]);
  ...
  async function handleSaveProfile() {
    try {
      ...
      if (avatarFile) {
        const uploadRes =
          await settingsService.uploadProfilePicture(avatarFile);

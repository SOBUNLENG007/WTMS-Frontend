...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const data: Notification[] = Array.isArray(rawPayload)
        ? rawPayload
        : Array.isArray(rawPayload?.content)
        ? rawPayload.content
        : [];
      const totalFromPayload =
        rawPayload && typeof rawPayload === "object" && "total" in rawPayload
          ? Number((rawPayload as { total?: number }).total || 0)
          : data.length;

      setNotifications(data);
      setTotal(totalFromPayload || data.length);
    } catch (err: unknown) {
      setError(
